...
/// KoalaBear^4 extension field.
///
/// This is the degree-2 extension `KB2[Y] / (Y^2 - X)` of [`kb2::Scalar`], where `X` is the KB2
/// generator (`X^2 = 3` in the base field). Equivalently, this is the degree-4 extension
/// `GF(p)[Y] / (Y^4 - 3)` of the KoalaBear field, where `p` is [`MODULUS`]. A scalar
/// `Scalar(w, x, y, z)` represents `KB2(w, x) * Y + KB2(y, z)`.
///
...
/// `Scalar::3` is the least significant. All coefficients are in Montgomery form, exactly like the
...
impl Scalar {
    /// Constructs a KoalaBear^4 scalar from the raw values of its coefficients, ie.
    /// `(w * X + x) * Y + (y * X + z)`.
    ///
    /// Panics if any coefficient exceeds [`MODULUS`].
    #[inline]
    pub const fn from_coefficients(w: u32, x: u32, y: u32, z: u32) -> Self {
        Self(
            base::Scalar::from_const(w).0,
            base::Scalar::from_const(x).0,
            base::Scalar::from_const(y).0,
            base::Scalar::from_const(z).0,
        )
    }
    ...
    #[inline]
    const fn to_raw(&self) -> (u32, u32, u32, u32) {
        (
    ...
    /// Constructs a KoalaBear^4 scalar from a raw 64-bit numeric value.
    #[inline]
    ...
    /// than `MODULUS^4`.
    fn try_from_value(value: u128) -> CtOption<Self> {
        const MODULUS_128: u128 = MODULUS as u128;
        let mut remaining = value;
        let z = remaining % MODULUS_128;
        remaining /= MODULUS_128;
        let y = remaining % MODULUS_128;
        remaining /= MODULUS_128;
        let x = remaining % MODULUS_128;
        remaining /= MODULUS_128;
        let w = remaining % MODULUS_128;
        remaining /= MODULUS_128;
        CtOption::new(
            Self(
                kb_to_montgomery(w as u32),
                kb_to_montgomery(x as u32),
                kb_to_montgomery(y as u32),
                kb_to_montgomery(z as u32),
            ),
            ((remaining == 0) as u8).into(),
        )
    }
}
...
impl Add<base::Scalar> for Scalar {
    ...
    fn add(self, rhs: base::Scalar) -> Self::Output {
        Self(self.0, self.1, self.2, kb_add(self.3, rhs.0))
    }
}
...
impl Add<kb2::Scalar> for Scalar {
    ...
    fn add(self, rhs: kb2::Scalar) -> Self::Output {
        Self(self.0, self.1, kb_add(self.2, rhs.0), kb_add(self.3, rhs.1))
    }
}
...
impl AddAssign<kb2::Scalar> for Scalar {
    fn add_assign(&mut self, rhs: kb2::Scalar) {
        self.2 = kb_add(self.2, rhs.0);
        self.3 = kb_add(self.3, rhs.1);
    }
}
...
impl Neg for Scalar {
    ...
    fn neg(self) -> Self::Output {
        let [c0, c1, c2, c3] = kb_sub4([0, 0, 0, 0], [self.0, self.1, self.2, self.3]);
        Self(c0, c1, c2, c3)
    }
}
...
impl Sub<base::Scalar> for Scalar {
    ...
    fn sub(self, rhs: base::Scalar) -> Self::Output {
        Self(self.0, self.1, self.2, kb_sub(self.3, rhs.0))
    }
}
...
impl Sub<kb2::Scalar> for Scalar {
    ...
    fn sub(self, rhs: kb2::Scalar) -> Self::Output {
        Self(self.0, self.1, kb_sub(self.2, rhs.0), kb_sub(self.3, rhs.1))
    }
}
...
impl SubAssign<kb2::Scalar> for Scalar {
    fn sub_assign(&mut self, rhs: kb2::Scalar) {
        self.2 = kb_sub(self.2, rhs.0);
        self.3 = kb_sub(self.3, rhs.1);
    }
}
...
impl Mul<kb2::Scalar> for Scalar {
    ...
    fn mul(self, rhs: kb2::Scalar) -> Self::Output {
        ...
        let [c0, c1] = kb_mul2([self.2, self.3], [rhs.0, rhs.1]);
        Self(y0, y1, c0, c1)
    }
}
...
impl From<u8> for Scalar {
    fn from(value: u8) -> Self {
        Self::from_coefficients(0, 0, 0, value as u32)
    }
}
...
impl TryFrom<u128> for Scalar {
    ...
    fn try_from(value: u128) -> Result<Self, Self::Error> {
        Self::try_from_value(value)
            .into_option()
            .ok_or_else(|| anyhow!("{:#x} exceeds the KoalaBear^4 range", value))
    }
}
...
impl TryFrom<U256> for Scalar {
    ...
    fn try_from(value: U256) -> Result<Self, Self::Error> {
        if value > U256::from(u128::MAX) {
            Err(anyhow!("{:#x} exceeds the KoalaBear^4 range", value))
        } else {
...
impl Field for Scalar {
    const MODULUS: &'static str = "0xf817e017d05fc017a060001fc000001";
    ...
    const ONE: Self = Self::from_coefficients(0, 0, 0, 1);

    const MAX: Self = Self::from_coefficients(MODULUS - 1, MODULUS - 1, MODULUS - 1, MODULUS - 1);

    const S: usize = 26;

    const MULTIPLICATIVE_GENERATOR: Self =
        Self::from_coefficients(0x3f2e9488, 0x31b56ca3, 0x46d95847, 0x6824d3f2);

    const MINUS_TWO: Self =
        Self::from_coefficients(MODULUS - 1, MODULUS - 1, MODULUS - 1, MODULUS - 2);

    const TWO_INV: Self = Self::from_coefficients(0, 0, 0, 0x3f800001);

    const ROOT_OF_UNITY: Self = Self::from_coefficients(0x2e5afdc8, 0, 0, 0);

    const ROOT_OF_UNITY_INV: Self = Self::from_coefficients(0, 0x09cf664f, 0, 0);

    const DELTA: Self = Self::from_coefficients(0x6feede38, 0x7b712f64, 0x58753d5f, 0x1bc50541);

    fn is_odd(&self) -> Choice {
        let (w, x, y, z) = self.to_raw();
        (((w ^ x ^ y ^ z) & 1) as u8).into()
    }
    ...
    fn invert(&self) -> CtOption<Self> {
        let a = kb2::Scalar(self.0, self.1);
        let b = kb2::Scalar(self.2, self.3);
        let a2 = a * a;
        let a2x = kb2::Scalar(
            a2.1,
        ...
        let norm = b * b - a2x;
        let conjugate = Self(kb_sub(0, self.0), kb_sub(0, self.1), self.2, self.3);
    ...
    fn try_from_le_bytes(bytes: &[u8]) -> CtOption<Self> {
        ...
        Self::try_from_value(u128::from_le_bytes(fixed_bytes))
    }

    fn try_from_be_bytes(bytes: &[u8]) -> CtOption<Self> {
        let mut fixed_bytes = [0u8; 16];
        fixed_bytes.copy_from_slice(bytes);
        Self::try_from_value(u128::from_be_bytes(fixed_bytes))
    }
...
impl Field128 for Scalar {
    ...
    fn from_u256_mod_n(u256: U256) -> Self {
        ...
        Self::try_from((u256 % modulus_pow4).as_u128()).unwrap()
    }
    ...
    fn try_to_u32(&self) -> CtOption<u32> {
        let value = self.to_u128();
        CtOption::new(value as u32, ((value <= u32::MAX as u128) as u8).into())
    }

    fn try_to_u64(&self) -> CtOption<u64> {
        let value = self.to_u128();
        CtOption::new(value as u64, ((value <= u64::MAX as u128) as u8).into())
    }

    fn to_u128(&self) -> u128 {
        const MODULUS_128: u128 = MODULUS as u128;
        let (w, x, y, z) = self.to_raw();
        ((w as u128 * MODULUS_128 + x as u128) * MODULUS_128 + y as u128) * MODULUS_128 + z as u128
    }
}
...
mod tests {
    ...
    #[inline]
    const fn from_coefficients(w: u32, x: u32, y: u32, z: u32) -> Scalar {
        Scalar::from_coefficients(w, x, y, z)
    }
    ...
    /// `MODULUS^4`, ie. one past the largest representable value.
    fn modulus_pow4() -> u128 {
    ...
    fn test_from_const() {
        ...
        assert_eq!(from_const(MODULUS as u64), from_coefficients(0, 0, 1, 0));
        assert_eq!(
            from_const(MODULUS as u64 + 1),
            from_coefficients(0, 0, 1, 1)
        );
        assert_eq!(
            from_const((MODULUS as u64) * (MODULUS as u64)),
            from_coefficients(0, 1, 0, 0)
        );
        assert_eq!(
            from_const(u64::MAX),
            from_coefficients(0, 4, 134746136, 402124771)
        );
    }

    #[test]
    fn test_from_coefficients() {
        assert_eq!(from_coefficients(0, 0, 0, 0), Scalar::ZERO);
        assert_eq!(from_coefficients(0, 0, 0, 1), Scalar::ONE);
        assert_eq!(
            from_coefficients(0, 0, 1, 2),
    ...
    fn test_from_coefficients_out_of_range() {
        from_coefficients(MODULUS, 0, 0, 0);
    }

    #[test]
    fn test_modulus() {
        assert_eq!(Scalar::MODULUS, "0xf817e017d05fc017a060001fc000001");
        assert_eq!(
            Scalar::MAX,
            from_coefficients(MODULUS - 1, MODULUS - 1, MODULUS - 1, MODULUS - 1)
        );
    }
    ...
    fn test_zero() {
        ...
        assert_eq!(Scalar::ZERO * from_coefficients(1, 2, 3, 4), Scalar::ZERO);
    }
    ...
    fn test_one() {
        ...
        assert_eq!(
            Scalar::ONE * from_coefficients(1, 2, 3, 4),
            from_coefficients(1, 2, 3, 4)
        );
    }
    ...
    fn test_multiplicative_generator() {
        assert_eq!(
            ...
            from_coefficients(0x3f2e9488, 0x31b56ca3, 0x46d95847, 0x6824d3f2)
        );
    ...
    fn test_minus_two() {
        ...
        let value = from_coefficients(7, 11, 13, 17);
    ...
    fn test_root_of_unity() {
        assert_eq!(
            ...
            kb2::Scalar::ROOT_OF_UNITY.into()
        );
        assert_eq!(Scalar::S, kb2::Scalar::S + 1);
        for i in 0..Scalar::S {
            assert_ne!(
    ...
    #[test]
    fn test_equality() {
        assert_eq!(from_coefficients(1, 2, 3, 4), from_coefficients(1, 2, 3, 4));
        assert_ne!(from_coefficients(1, 2, 3, 4), from_coefficients(1, 2, 3, 5));
    }

    #[test]
    fn test_total_order() {
        let v0 = from_coefficients(0, 0, 0, 0);
        let v1 = from_coefficients(0, 0, 0, 1);
        let v2 = from_coefficients(0, 0, 1, 0);
        let v3 = from_coefficients(0, 1, 0, 0);
        let v4 = from_coefficients(1, 0, 0, 0);
    ...
    #[test]
    fn test_ct_eq() {
        let a = from_coefficients(1, 2, 3, 4);
        let b = from_coefficients(1, 2, 3, 4);
        let c = from_coefficients(1, 2, 3, 5);
        let d = from_coefficients(9, 2, 3, 4);
    ...
    fn test_ct_gt() {
        let v0 = from_coefficients(0, 0, 0, 0);
        let v1 = from_coefficients(0, 0, 0, 42);
        let v2 = from_coefficients(0, 0, 1, 0);
    ...
    fn test_conditional_select() {
        let a = from_coefficients(1, 2, 3, 4);
        let b = from_coefficients(5, 6, 7, 8);
    ...
    #[test]
    fn test_add() {
        let lhs = from_coefficients(1, 2, 3, 4);
        let rhs = from_coefficients(5, 6, 7, 8);
        assert_eq!(lhs + rhs, from_coefficients(6, 8, 10, 12));
        assert_eq!(lhs + &rhs, from_coefficients(6, 8, 10, 12));
    }

    #[test]
    fn test_add_wraparound() {
        let lhs = from_coefficients(MODULUS - 1, MODULUS - 2, MODULUS - 3, MODULUS - 4);
        let rhs = from_coefficients(2, 3, 4, 5);
        assert_eq!(lhs + rhs, from_coefficients(1, 1, 1, 1));
    }

    #[test]
    fn test_add_assign() {
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs += from_coefficients(5, 6, 7, 8);
        assert_eq!(lhs, from_coefficients(6, 8, 10, 12));
    }
    ...
    fn test_add_base_scalar() {
        ...
        assert_eq!(lhs + rhs, from_coefficients(1, 2, 3, 9));
        assert_eq!(lhs + &rhs, from_coefficients(1, 2, 3, 9));
    }
    ...
    fn test_add_assign_base_scalar() {
        ...
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs += rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 9));
    ...
    fn test_add_kb2() {
        ...
        assert_eq!(lhs + rhs, from_coefficients(1, 2, 8, 10));
        assert_eq!(lhs + &rhs, from_coefficients(1, 2, 8, 10));
    }
    ...
    fn test_add_assign_kb2() {
        ...
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs += rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 8, 10));
    ...
    fn test_add_kb8() {
        let lhs = from_coefficients(1, 2, 3, 4);
        let rhs = kb8::Scalar::from_coefficients(5, 6, 7, 8, 9, 10, 11, 12);
        let expected = kb8::Scalar::from_coefficients(5, 6, 7, 8, 10, 12, 14, 16);
    ...
    fn test_neg() {
        ...
        assert_eq!(
            -from_coefficients(1, 2, 3, 4),
            from_coefficients(MODULUS - 1, MODULUS - 2, MODULUS - 3, MODULUS - 4)
        );
        assert_eq!(
            from_coefficients(1, 2, 3, 4) + -from_coefficients(1, 2, 3, 4),
            Scalar::ZERO
        );
    }
    ...
    #[test]
    fn test_sub() {
        let lhs = from_coefficients(6, 8, 10, 12);
        let rhs = from_coefficients(5, 6, 7, 8);
        assert_eq!(lhs - rhs, from_coefficients(1, 2, 3, 4));
        assert_eq!(lhs - &rhs, from_coefficients(1, 2, 3, 4));
    }

    #[test]
    fn test_sub_wraparound() {
        let lhs = from_coefficients(1, 1, 1, 1);
        let rhs = from_coefficients(2, 3, 4, 5);
        assert_eq!(
            lhs - rhs,
            from_coefficients(MODULUS - 1, MODULUS - 2, MODULUS - 3, MODULUS - 4)
    ...
    #[test]
    fn test_sub_assign() {
        let mut lhs = from_coefficients(6, 8, 10, 12);
        lhs -= from_coefficients(5, 6, 7, 8);
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_sub_base_scalar() {
        let lhs = from_coefficients(1, 2, 3, 9);
        let rhs = base::Scalar::from_const(5);
        assert_eq!(lhs - rhs, from_coefficients(1, 2, 3, 4));
        assert_eq!(lhs - &rhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_sub_assign_base_scalar() {
        ...
        let mut lhs = from_coefficients(1, 2, 3, 9);
        lhs -= rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    ...
    #[test]
    fn test_sub_kb2() {
        let lhs = from_coefficients(1, 2, 8, 10);
        let rhs = kb2::Scalar::from_coefficients(5, 6);
        assert_eq!(lhs - rhs, from_coefficients(1, 2, 3, 4));
        assert_eq!(lhs - &rhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_sub_assign_kb2() {
        ...
        let mut lhs = from_coefficients(1, 2, 8, 10);
        lhs -= rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    ...
    fn test_extension_root() {
        let y = from_coefficients(0, 1, 0, 0);
        let y_squared = y * y;
        // Y^2 = X, i.e. the KB2 generator embedded with a zero Y-coefficient.
        assert_eq!(y_squared, from_coefficients(0, 0, 1, 0));
        assert_eq!(y * &y, y_squared);
        // Y^4 = QUADRATIC_NON_RESIDUE.
        assert_eq!(
            y_squared * y_squared,
            from_const(QUADRATIC_NON_RESIDUE as u64)
        );
    }
    ...
    fn test_mul_by_zero() {
        assert_eq!(Scalar::ZERO * from_coefficients(1, 2, 3, 4), Scalar::ZERO);
        assert_eq!(from_coefficients(1, 2, 3, 4) * Scalar::ZERO, Scalar::ZERO);
    }

    #[test]
    fn test_mul_by_one() {
        assert_eq!(
            Scalar::ONE * from_coefficients(2, 3, 4, 5),
            from_coefficients(2, 3, 4, 5)
        );
        assert_eq!(
            from_coefficients(2, 3, 4, 5) * Scalar::ONE,
            from_coefficients(2, 3, 4, 5)
        );
    }
    ...
    fn test_mul() {
        let lhs = from_coefficients(1, 2, 3, 4);
        let rhs = from_coefficients(5, 6, 7, 8);
        let expected = from_coefficients(60, 106, 79, 143);
        assert_eq!(lhs * rhs, expected);
        assert_eq!(lhs * &rhs, expected);
    ...
    fn test_mul_assign() {
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs *= from_coefficients(5, 6, 7, 8);
        assert_eq!(lhs, from_coefficients(60, 106, 79, 143));
    }
    ...
    fn test_mul_base_scalar() {
        let lhs = from_coefficients(1, 2, 3, 4);
        let rhs = base::Scalar::from_const(5);
        assert_eq!(lhs * rhs, from_coefficients(5, 10, 15, 20));
        assert_eq!(lhs * &rhs, from_coefficients(5, 10, 15, 20));
    }
    ...
    fn test_mul_assign_base_scalar() {
        ...
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs *= rhs;
        assert_eq!(lhs, from_coefficients(5, 10, 15, 20));
    ...
    fn test_mul_kb2() {
        let lhs = from_coefficients(1, 2, 3, 4);
        let rhs = kb2::Scalar::from_coefficients(5, 6);
        assert_eq!(lhs * rhs, from_coefficients(16, 27, 38, 69));
        assert_eq!(lhs * &rhs, from_coefficients(16, 27, 38, 69));
    }
    ...
    fn test_mul_assign_kb2() {
        ...
        let mut lhs = from_coefficients(1, 2, 3, 4);
        lhs *= rhs;
        assert_eq!(lhs, from_coefficients(16, 27, 38, 69));
    ...
    #[test]
    fn test_mul_kb8() {
        let lhs = from_coefficients(5, 6, 7, 8);
        let rhs = kb8::Scalar::from_coefficients(1, 2, 3, 4, 5, 6, 7, 8);
        let expected = kb8::Scalar::from_coefficients(60, 106, 79, 143, 164, 306, 223, 391);
        assert_eq!(lhs * rhs, expected);
        assert_eq!(lhs * &rhs, expected);
    }
    ...
    fn test_div_by_one() {
        assert_eq!(
            from_coefficients(1, 2, 3, 4) / Scalar::ONE,
            from_coefficients(1, 2, 3, 4)
        );
        assert_eq!(
            from_coefficients(1, 2, 3, 4) / &Scalar::ONE,
    ...
    fn test_div_assign() {
        let rhs = from_coefficients(5, 6, 7, 8);
        let mut lhs = from_coefficients(1, 2, 3, 4) * rhs;
        lhs /= rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_div_base_scalar() {
        let lhs = from_coefficients(5, 10, 15, 20);
        let rhs = base::Scalar::from_const(5);
        assert_eq!(lhs / rhs, from_coefficients(1, 2, 3, 4));
        assert_eq!(lhs / &rhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_div_assign_base_scalar() {
        ...
        let mut lhs = from_coefficients(5, 10, 15, 20);
        lhs /= rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    ...
    #[test]
    fn test_div_kb2() {
        let lhs = from_coefficients(16, 27, 38, 69);
        let rhs = kb2::Scalar::from_coefficients(5, 6);
        assert_eq!(lhs / rhs, from_coefficients(1, 2, 3, 4));
        assert_eq!(lhs / &rhs, from_coefficients(1, 2, 3, 4));
    }
    ...
    fn test_div_assign_kb2() {
        ...
        let mut lhs = from_coefficients(16, 27, 38, 69);
        lhs /= rhs;
        assert_eq!(lhs, from_coefficients(1, 2, 3, 4));
    ...
    fn test_inversion() {
        ...
        test_inversion_impl(from_coefficients(0, 0, 0, 42));
        test_inversion_impl(from_coefficients(1, 0, 0, 0));
        test_inversion_impl(from_coefficients(0, 1, 0, 0));
        test_inversion_impl(from_coefficients(0, 0, 1, 0));
        test_inversion_impl(from_coefficients(7, 11, 13, 17));
    ...
    fn test_invert_batch() {
        let values = vec![
            from_coefficients(1, 2, 3, 4),
            from_coefficients(0, 0, 0, 42),
            Scalar::ONE,
            from_coefficients(3, 5, 7, 9),
        ];
    ...
    fn test_sum() {
        let values = vec![
            from_coefficients(1, 2, 3, 4),
            from_coefficients(5, 6, 7, 8),
            from_coefficients(1, 1, 1, 1),
        ];
        assert_eq!(
            values.iter().sum::<Scalar>(),
            from_coefficients(7, 9, 11, 13)
        );
        assert_eq!(
            values.into_iter().sum::<Scalar>(),
            from_coefficients(7, 9, 11, 13)
        );
    }
    ...
    fn test_product() {
        let values = vec![
            from_coefficients(0, 0, 0, 2),
            from_coefficients(0, 0, 0, 3),
            from_coefficients(0, 0, 0, 4),
        ];
        assert_eq!(
            values.iter().product::<Scalar>(),
            from_coefficients(0, 0, 0, 24)
        );
        assert_eq!(
    ...
    fn test_fmt_display() {
        ...
        assert_eq!(
            ...
            "0x00000000000000000000000013371337"
        );
        assert_eq!(
            format!("{}", from_coefficients(1, 2, 3, 4)),
            "0x000000001f417f013b050004f600000a"
        );
        assert_eq!(
            format!("{}", Scalar::MAX),
            "0x0f817e017d05fc017a060001fc000000"
        );
    }
    ...
    fn test_fmt_lower_hex() {
        ...
        assert_eq!(
            format!("{:x}", from_coefficients(1, 2, 3, 4)),
            "1f417f013b050004f600000a"
        );
        assert_eq!(
            format!("{:x}", Scalar::MAX),
            "f817e017d05fc017a060001fc000000"
        );
    }
    ...
    fn test_from_str() {
        ...
        assert_eq!(
            "0xf817e017d05fc017a060001fc000000"
                .parse::<Scalar>()
                .unwrap(),
            Scalar::MAX
        );
    ...
    fn test_from_str_invalid() {
        ...
        assert!(
            "0xf817e017d05fc017a060001fc000001"
                .parse::<Scalar>()
                .is_err()
        );
        // MODULUS^4, i.e. one past the largest representable value.
        assert!(
            "20610781933055814402734644169665413121"
                .parse::<Scalar>()
                .is_err()
        );
    }
    ...
    fn test_display_from_str_roundtrip() {
        let value = from_coefficients(1, 2, 3, 4);
    ...
    fn test_from_u32() {
        ...
        assert_eq!(Scalar::from(u32::MAX), from_coefficients(0, 0, 2, 33554429));
    }
    ...
    fn test_from_u64() {
        ...
        assert_eq!(Scalar::from(MODULUS as u64), from_coefficients(0, 0, 1, 0));
        assert_eq!(
            Scalar::from(u64::MAX),
            from_coefficients(0, 4, 134746136, 402124771)
        );
    ...
    fn test_try_from_u128() {
        ...
        let modulus = MODULUS as u128;
        assert_eq!(
            Scalar::try_from(modulus * modulus).unwrap(),
            from_coefficients(0, 1, 0, 0)
        );
        assert_eq!(
            Scalar::try_from(modulus * modulus * modulus).unwrap(),
            from_coefficients(1, 0, 0, 0)
        );
        assert_eq!(Scalar::try_from(modulus_pow4() - 1).unwrap(), Scalar::MAX);
        assert!(Scalar::try_from(modulus_pow4()).is_err());
        assert!(Scalar::try_from((1u128 << 127) + 12345).is_err());
    ...
    fn test_try_from_u256() {
        ...
        assert_eq!(
            Scalar::try_from(U256::from(modulus_pow4() - 1)).unwrap(),
            Scalar::MAX
        );
        assert!(Scalar::try_from(U256::from(modulus_pow4())).is_err());
    ...
    #[test]
    fn test_is_even() {
        assert!(bool::from(from_coefficients(0, 0, 0, 0).is_even()));
        assert!(bool::from(from_coefficients(1, 1, 0, 0).is_even()));
        assert!(!bool::from(from_coefficients(0, 0, 0, 1).is_even()));
        assert!(!bool::from(from_coefficients(1, 0, 0, 0).is_even()));
    }

    #[test]
    fn test_is_odd() {
        assert!(!bool::from(from_coefficients(0, 0, 0, 0).is_odd()));
        assert!(!bool::from(from_coefficients(1, 1, 0, 0).is_odd()));
        assert!(bool::from(from_coefficients(0, 0, 0, 1).is_odd()));
        assert!(bool::from(from_coefficients(1, 0, 0, 0).is_odd()));
    }
    ...
    fn test_double() {
        assert_eq!(
            from_coefficients(1, 2, 3, 4).double(),
            from_coefficients(2, 4, 6, 8)
        );
    }
    ...
    fn test_square() {
        assert_eq!(
            from_coefficients(0, 0, 0, 5).square(),
            from_coefficients(0, 0, 0, 25)
        );
    }
    ...
    fn test_integer_division_multi_word() {
        ...
        let quotient = from_coefficients(0, 2, 278497010, 233583434);
        let remainder = from_const(35060824);
    ...
    fn test_try_from_le_bytes() {
        assert_eq!(
            Scalar::try_from_le_bytes(&42u128.to_le_bytes()).unwrap(),
            from_const(42)
        );
        assert!(bool::from(
    ...
    fn test_try_from_be_bytes() {
        assert_eq!(
            Scalar::try_from_be_bytes(&42u128.to_be_bytes()).unwrap(),
            from_const(42)
        );
        assert!(bool::from(
            Scalar::try_from_be_bytes(&[255u8; 16]).is_none()
        ));
    }
    ...
    fn test_try_to_u8() {
        ...
        assert!(from_coefficients(1, 0, 0, 0).try_to_u8().is_none());
    }
    ...
    fn test_try_to_u16() {
        ...
        assert!(from_coefficients(1, 0, 0, 0).try_to_u16().is_none());
    }

    #[test]
    fn test_field128_to_le_bytes() {
        let value = from_coefficients(1, 2, 3, 4);
        assert_eq!(value.to_le_bytes(), value.to_u128().to_le_bytes());
    }

    #[test]
    fn test_field128_to_be_bytes() {
        let value = from_coefficients(1, 2, 3, 4);
        assert_eq!(value.to_be_bytes(), value.to_u128().to_be_bytes());
    }
    ...
    fn test_from_u256_mod_n() {
        ...
        assert_eq!(
            ...
            from_coefficients(0, 0, 1, 0)
        );
        let modulus_pow4 = U256::from(modulus_pow4());
        assert_eq!(Scalar::from_u256_mod_n(modulus_pow4), Scalar::ZERO);
        assert_eq!(
            Scalar::from_u256_mod_n(modulus_pow4 + U256::from(1)),
            Scalar::ONE
        );
        assert_eq!(
            Scalar::from_u256_mod_n(U256::from((1u128 << 127) + 12345)),
            from_coefficients(543245225, 1801733882, 1897641743, 555675263)
        );
        assert_eq!(
            Scalar::from_u256_mod_n(U256::MAX),
            Scalar::try_from((U256::MAX % modulus_pow4).as_u128()).unwrap()
        );
    }
    ...
    fn test_field128_try_to_u32() {
        ...
        assert_eq!(from_coefficients(0, 0, 1, 0).try_to_u32().unwrap(), MODULUS);
        assert!(bool::from(
            from_const(u32::MAX as u64 + 1).try_to_u32().is_none()
        ));
        assert!(bool::from(
            from_coefficients(1, 0, 0, 0).try_to_u32().is_none()
        ));
    }

    #[test]
    fn test_field128_try_to_u64() {
        ...
        assert_eq!(
            from_coefficients(0, 0, MODULUS - 1, MODULUS - 1)
                .try_to_u64()
                .unwrap(),
            (MODULUS as u64) * (MODULUS as u64) - 1
        );
        assert!(bool::from(
            from_coefficients(0, 5, 0, 0).try_to_u64().is_none()
        ));
        assert!(bool::from(
            from_coefficients(1, 0, 0, 0).try_to_u64().is_none()
        ));
    }
    ...
    fn test_field128_to_u128() {
        ...
        assert_eq!(
            from_coefficients(1, 2, 3, 4).to_u128(),
            0x1f417f013b050004f600000a
        );
        assert_eq!(Scalar::MAX.to_u128(), modulus_pow4() - 1);
    }
    ...
    fn test_field128_to_u256() {
        ...
        assert_eq!(Scalar::MAX.to_u256(), U256::from(modulus_pow4() - 1));
    }
    ...
    fn test_field128_to_u512() {
        ...
        assert_eq!(Scalar::MAX.to_u512(), U512::from(modulus_pow4() - 1));
    }
}
